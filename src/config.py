...
def _load_aliyun_llm_config() -> Optional[LLMConfig]:
    env_key = (
    ...
    model = (env_model or DEFAULT_ALIYUN_LLM_MODEL).strip()
...
def load_llm_configs(
    *,
    ...
    aliyun_cfg = _load_aliyun_llm_config()
    if aliyun_cfg:
        configs.append(aliyun_cfg)
