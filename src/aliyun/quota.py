...
def _classify_model(model: str) -> str:
    ...
    if not name:
        return "unknown"
    if name.startswith(("wan", "wanx", "qwen-image", "z-image")) or "image" in name:
        return "image"
    return "llm"
...
def _parse_quota_values(raw_text: str) -> tuple[int | float | None, int | float | None, int | float | None]:
    total = _number_after_labels(raw_text, ["总额度", "总量", "免费额度", "额度总量", "总计"])
...
def run_collect_aliyun_quota_sync(
    *,
    ...
    model_names = _dedupe(models or aliyun_quota_model_candidates())
            ...
            try:
                ...
                while time.time() < deadline:
                    ...
                    if any(model.lower() in body_text.lower() for model in model_names):
                        break
                    if any(hint in body_text for hint in ("免费额度", "模型名称", "剩余额度", "额度")):
                        break
                ...
                records = parse_aliyun_quota_text(raw_text, model_names, source_url=quota_url)
