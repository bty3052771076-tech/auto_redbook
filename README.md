...
依赖安装后运行以下命令。构建完成后，也可以直接双击 `Start-Web-GUI.cmd`，它会启动本地服务并用系统默认浏览器打开工作台：
...
首次启动建议先使用测试模式或 `--count 1` 验证配置。生成、删除、发布属于真实平台操作；正式执行前应确认平台、标题、图片和正文，并留意短信验证、草稿校验和平台风控。
...
4. 确认无误后再提高数量。普通新闻优先 AI 生图，生图失败才按配置回退 Pexels；每日 AI 讯息使用本地简报卡片，不会因为缺少普通新闻配图而失败。
...
2026-09-06追加修复国际争议识别、必选查询被截断和BBC仅取体育频道的问题；正文按配额缺口优先审核，重复标题不能充当充分正文，并增加审核批次、逐条耗时和拒绝原因记录。此次追加修改按要求未运行测试。此前的 `380 passed in 175.71s` 仅属于追加修复前的回归结果，不能视为当前代码已验证；真实新闻、模型、生图和创作者中心上传仍需实机验证。

后续测试结果（2026-09-06）：相关回归 `282 passed in 117.98s`。10条实机计时为21分59.657秒，只生成6条本地草稿，因国内配额及批次完整性未满足而未上传，不能宣称端到端提速已通过。主要耗时在正文获取与审核，以及生成后才进行的配额淘汰；部分失败批次的CSV生成数量也有待修正。
