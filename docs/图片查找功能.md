...
- `IMAGE_PROVIDER`：`pexels`（当前已实现；后续可扩展 `unsplash` / `wikimedia` 等）
- 默认策略：
...
- 环境变量（推荐）：
  ...
  - `IMAGE_PROVIDER`：图片源（当前支持 `pexels`）
...
## 进度跟踪
当前状态：已完成 Pexels 自动配图（无素材时自动搜索并下载到 post assets，元数据落盘），待补充 README 配置说明与端到端验证。
