...
- 不做复杂的生成式图片（AIGC）编排（多风格/多分镜/并发队列）；仅提供“阿里云百炼 API 生图”作为可选 provider。
...
- `IMAGE_PROVIDER`：
  - `pexels`：检索下载（当前已实现）
  - `aliyun`：阿里云百炼 / DashScope API 生图（当前已实现；更稳定，不依赖网页）
  - 后续可扩展：`unsplash` / `wikimedia` 等
- 默认策略：
...
说明：
- 对于 `pexels`：会保存 `license/author/source_page_url` 等可追溯信息。
- 对于 `aliyun`：属于生成式图片，通常不包含第三方署名字段；会保存 `model/size/prompt/src_url/request_id` 等审计信息。
...
- 环境变量（推荐）：
  - `AUTO_IMAGE`：是否启用自动配图（默认启用；设为 `0/false/off` 关闭）
  - `IMAGE_PROVIDER`：图片源（当前支持 `pexels` / `aliyun`）
  - `PEXELS_API_KEY`：Pexels API key（或使用本机 `docs/pexels_api-key.md`）
  - `PEXELS_BASE_URL`：Pexels base_url（默认 `https://api.pexels.com`）
  - `ALIYUN_IMAGE_API_KEY`：阿里云百炼 API key（或使用本机 `docs/aliyun_image_api-key.md`）
  - `ALIYUN_IMAGE_MODEL`：生图模型（默认 `qwen-image-plus`）
  - `ALIYUN_IMAGE_SIZE`：尺寸（默认 `1104*1472`）
  - `IMAGE_TIMEOUT_S`：请求超时秒数（默认 20）
  - `IMAGE_MAX_CANDIDATES`：搜索候选数量（默认 30）
  - `IMAGE_ORIENTATION`：图片方向（默认 `portrait`）
  - `IMAGE_QUERY_DEFAULT`：兜底 query（默认 `lifestyle`）
- 或本机配置文件（被 `.gitignore` 忽略）：
  - `docs/pexels_api-key.md`（可参考 `docs/pexels_api-key.example.md`）
...
## 进度跟踪
当前状态：已完成 Pexels 检索配图 + 阿里云百炼 API 生图（无素材时自动生成/下载到 post assets，元数据落盘）。

- [x] 选定图片 API provider（Pexels）
- [ ] 明确许可与署名要求（是否需要写入正文/仅存元数据）
- [x] 设计并落地 provider（Pexels：search/pick/download）
- [x] 设计并落地 provider（Aliyun：text-to-image 生成 + 下载落盘）
- [x] CLI：当无图片时自动触发补图（可用 `AUTO_IMAGE=0` 关闭）
- [x] 存储与元数据落盘（`post.platform.image` 记录可追溯信息）
- [x] Playwright：上传会使用自动下载图片（保存到 `data/posts/<id>/assets/`）
- [x] 单测（query 生成 / pick 逻辑；不走真实网络）
- [x] 更新 README（Pexels 配置说明、注意事项）
- [x] 端到端验证：无图片 `apps.cli run/auto` 可保存草稿（需手动登录一次）
