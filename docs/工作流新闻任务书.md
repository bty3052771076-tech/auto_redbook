...
- 若提供 `prompt`：
  ...
  - 对候选新闻进行相关性评分（标题命中权重更高），选取得分最高的一条
- 若未提供 `prompt`：
  - 随便挑选 3 条新闻（默认取候选列表前 3 条），分别生成并保存 3 条草稿
...
- 不提供 `prompt` 时，会生成并保存 3 条“每日新闻”草稿（每条对应不同 picked 新闻）
