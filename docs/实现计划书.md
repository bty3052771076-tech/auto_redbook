...
- [x] CLI 新增 `--count` 控制单次生成数量，取消“每日新闻”无提示词默认 3 条
- [ ] 批量 `auto --count 10` 已生成 10 条 post，但保存草稿阶段超时（需更长 `--wait-timeout`/确认登录）
...
- [x] AI 生图：新增 `IMAGE_PROVIDER=chatgpt_images`（ChatGPT Images 自动生图下载原图落盘，避免“截图截到模糊/生成中”）+ 探测脚本 `apps/inspect_chatgpt_images.py`
- [x] Cloudflare 拦截处理：自动化被 Cloudflare 卡住时，支持降级为“手动生图”（打开普通 Chrome 粘贴提示词生成，下载到 `data/posts/<post_id>/assets/` 后继续）
- [x] CDP 复用浏览器：支持 `CHATGPT_CDP_URL` / `XHS_CDP_URL` 连接到你已打开并通过校验的 Chrome，避免重新启动/跳转触发 Cloudflare，且避免 profile 被占用导致无法保存草稿
- [x] 修复：ChatGPT 生图阶段 UI 会对预览图加模糊层；落盘改为下载图片字节（支持 `http/blob/data` 源），不再依赖 UI 截图
- [ ] AI 生图端到端验证：每日新闻无图时走 ChatGPT 生图（自动 or 手动降级）并成功上传保存草稿
