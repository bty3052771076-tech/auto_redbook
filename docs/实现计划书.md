...
- [x] AI 生图：新增 `IMAGE_PROVIDER=aliyun`（阿里云百炼 / DashScope API 生图 + 下载落盘），替代网页式生图方案
- [x] 生图失败重试：超时/网络错误自动重试，超过上限放弃该条图片并继续后续新闻（避免整批卡死）
- [x] CDP 复用浏览器：保留 `XHS_CDP_URL` 连接到你已打开的 Chrome（可选），避免 profile 被占用导致无法保存草稿
- [x] 修复：生图落盘统一走“下载图片字节”，不依赖 UI 截图（避免截到模糊/生成中）
- [ ] AI 生图端到端验证：每日新闻无图时走 `aliyun` 生图并成功上传保存草稿
