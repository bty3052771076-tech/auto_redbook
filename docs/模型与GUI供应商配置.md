...
- `自动发帖`：一键执行 `create -> approve -> run`，并在同一界面选择 LLM 供应商、LLM 模型、配图来源、阿里云生图模型和每日新闻评价视角。
- `仅生成`：只生成本地草稿，便于检查文案和图片；每日新闻同样支持填写评价视角。
...
`qwen-image-2.0-pro-2026-04-22` 已加入 GUI 下拉列表，供手动测试；默认模型仍保持 `wan2.7-image`，避免影响现有批量自动发帖。
