...
- 配图重复：批量生成时不同草稿可能命中相同图片，影响区分度。
- 配图数量：自动配图仅 1 张，覆盖面不足。
...
- 配图：自动配图默认生成 3 张，并尽量避免同批次草稿重复命中同一张图。
...
### 6) 配图精准与多图扩展
- [x] Pexels 查询词：压缩冗余英文标题，补充关键词提取与去噪
- [x] Pexels 结果选择：相关性评分 + 去重挑选 3 张
- [x] 元数据：支持多图落盘（保留主图字段 + 追加 images 列表）
- [x] 可配置：支持 `AUTO_IMAGE_COUNT` 控制自动配图张数
...
最新进展（修复）：进一步过滤含“新闻”的话题词，避免 query 被“news”泛化；同时在每日新闻多条生成分支也改为优先使用 picked 标题/摘要作为配图提示词。
最新进展（测试）：重跑 `auto --title "每日新闻"`，3 条草稿均保存成功；查询不再包含“新闻/news”，但仍有个别结果偏离正文（例如政治新闻命中体育场图片）。
最新进展（修复）：已加入英文长标题压缩与国家/行业关键词映射，用于减少英文长句带来的误检，待下一轮 auto 验证。
最新进展（测试）：再次 auto 后草稿保存成功；“黑客文化回顾”配图较贴合，但“委内瑞拉石油归属存疑/全球新年热点速览”仍偏离（手部油/墓园），说明 Pexels 结果仍受英文标题噪声影响，需进一步压缩/关键词提纯。
最新进展（测试）：尝试 `auto --title "每日新闻" --count 10 --assets-glob "empty/pics/*"` 触发自动配图，但运行超时，尚未完成草稿与配图验证。
最新进展（完成）：Pexels 查询关键词压缩/去噪，自动配图默认 3 张并去重；`AUTO_IMAGE_COUNT/IMAGE_MIN_SCORE` 可调；`platform.images` 记录多图元数据；`pytest -q` 全部通过（29 passed）。
