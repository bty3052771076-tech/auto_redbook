...
export function DeleteDrafts({ jobs, submit, onError }: Actions & { jobs: Job[] }) {
  ...
  return <section className="tool-surface">
    ...
    <div className="toolbar"><button onClick={() => submit({ kind: "delete-preview", ...scope }).catch(e => onError(String(e)))}><Eye size={16}/>预览平台草稿</button>
...
export function SourceHealth({ submit, onError }: Actions) {
  ...
  return <section className="tool-surface">
    ...
    <div className="toolbar"><button className="primary" onClick={()=>submit({kind:"check-sources",collection,keywords,max_age_days:days}).catch(e=>onError(String(e)))}><RefreshCw size={16}/>检查信源</button>
    ...
    <div className="table-wrap"><table><thead><tr><th>信源</th><th>状态</th><th>材料 / 有日期</th><th>耗时</th><th>检查时间</th><th>错误</th></tr></thead><tbody>
...
export function AnalysisReport({ submit, onError }: Actions) {
  ...
  useEffect(()=>{void refresh();},[]);
  return <section className="tool-surface"><div className="section-heading"><h2>选题分析</h2><span>{dateLabel(report.captured_at)}</span></div>
...
export function LocalConfiguration({onError}: Pick<Actions,"onError">) {
  ...
  return <section className="tool-surface"><div className="section-heading"><h2>本机配置</h2><button onClick={refresh}><RefreshCw size={16}/>重新加载</button></div>
    <details><summary>模型与信源密钥</summary><div className="config-fields">{Object.entries(data.secrets).map(([key,configured])=><label className="field" key={key}>
